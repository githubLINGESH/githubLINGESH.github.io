...
var import_shared21 = require("@clerk/shared");
...
var errorThrower = (0, import_shared2.buildErrorThrower)({ packageName: "@clerk/backend" });
var { isDevOrStagingUrl } = (0, import_shared3.createDevOrStagingUrlCache)();

// src/api/endpoints/InterstitialApi.ts
var InterstitialAPI = class extends AbstractAPI {
  async getInterstitial() {
    (0, import_shared.deprecated)(
      "getInterstitial()",
...
var OrganizationAPI = class extends AbstractAPI {
  ...
  async getPendingOrganizationInvitationList(params) {
    (0, import_shared5.deprecated)("getPendingOrganizationInvitationList", "Use `getOrganizationInvitationList` instead.");
    const { organizationId, limit, offset } = params;
    this.requireId(organizationId);
    return this.request({
      method: "GET",
...
(0, import_shared.deprecatedProperty)(ExternalAccount, "picture", "Use `imageUrl` instead.");
...
(0, import_shared.deprecatedProperty)(Organization, "logoUrl", "Use `imageUrl` instead.");
...
(0, import_shared.deprecatedProperty)(OrganizationMembershipPublicUserData, "profileImageUrl", "Use `imageUrl` instead.");
...
(0, import_shared.deprecatedProperty)(User, "profileImageUrl", "Use `imageUrl` instead.");
...
function buildRequest(options) {
  const request = async (requestOptions) => {
    ...
    if (apiKey) {
      (0, import_shared.deprecated)("apiKey", "Use `secretKey` instead.");
    }
    if (httpOptions) {
      (0, import_shared.deprecated)(
        "httpOptions",
...
var import_shared11 = require("@clerk/shared");
...
function signedInAuthObject(sessionClaims, options, debugData) {
  ...
  if (apiKey) {
    (0, import_shared11.deprecated)("apiKey", "Use `secretKey` instead.");
  }
...
function loadInterstitialFromLocal(options) {
  if (options.frontendApi) {
    (0, import_shared.deprecated)("frontendApi", "Use `publishableKey` instead.");
  }
  if (options.pkgVersion) {
    (0, import_shared.deprecated)("pkgVersion", "Use `clerkJSVersion` instead.");
  }
  options.frontendApi = (0, import_shared.parsePublishableKey)(options.publishableKey)?.frontendApi || options.frontendApi || "";
  const domainOnlyInProd = !isDevOrStagingUrl(options.frontendApi) ? (0, import_shared.addClerkPrefix)(options.domain) : "";
  const {
    debugData,
    frontendApi,
    pkgVersion,
    ...
    <body>
        <script>
            ...
            (() => {
                ...
                script.async = true;
                script.src = '${clerkJSUrl || (0, import_shared.getScriptUrl)(proxyUrl || domainOnlyInProd || frontendApi, {
    pkgVersion,
    clerkJSVersion
  })}';
...
async function loadInterstitialFromBAPI(options) {
  ...
  const response = await (0, import_shared.callWithRetry)(
...
function buildPublicInterstitialUrl(options) {
  ...
  url.searchParams.append("clerk_js_version", clerkJSVersion || (0, import_shared.getClerkJsMajorVersionOrTag)(frontendApi, pkgVersion));
  if (publishableKey) {
...
var import_shared14 = require("@clerk/shared");
...
var import_shared13 = require("@clerk/shared");
function pemToBuffer(secret) {
  const trimmed = secret.replace(/-----BEGIN.*?-----/g, "").replace(/-----END.*?-----/g, "").replace(/\s/g, "");
  const decoded = (0, import_shared13.isomorphicAtob)(trimmed);
...
function decodeJwt(token) {
  ...
  (0, import_shared14.deprecatedObjectProperty)(
    payload,
    "orgs",
...
async function verifyJwt(token, { audience, authorizedParties, clockSkewInSeconds, clockSkewInMs, issuer, key }) {
  if (clockSkewInSeconds) {
    (0, import_shared.deprecated)("clockSkewInSeconds", "Use `clockSkewInMs` instead.");
  }
...
async function loadClerkJWKFromRemote({
  ...
  if (skipJwksCache || shouldRefreshCache) {
    ...
    const { keys } = await (0, import_shared.callWithRetry)(fetcher);
    if (!keys || !keys.length) {
      throw new TokenVerificationError({
        action: "Contact support@clerk.com" /* ContactSupport */,
...
var nonBrowserRequestInDevRule = (options) => {
  ...
  const key = secretKey || apiKey || "";
  if ((0, import_shared.isDevelopmentFromApiKey)(key) && !isBrowser(userAgent)) {
...
var isPrimaryInDevAndRedirectsToSatellite = (options) => {
  ...
  const key = secretKey || apiKey || "";
  const isDev = (0, import_shared.isDevelopmentFromApiKey)(key);
...
var potentialFirstLoadInDevWhenUATMissing = (options) => {
  ...
  const key = secretKey || apiKey || "";
  const res = (0, import_shared.isDevelopmentFromApiKey)(key);
  if (res && !clientUat) {
...
var potentialRequestAfterSignInOrOutFromClerkHostedUiInDev = (options) => {
  ...
  const key = secretKey || apiKey || "";
  if ((0, import_shared.isDevelopmentFromApiKey)(key) && crossOriginReferrer) {
...
var potentialFirstRequestOnProductionEnvironment = (options) => {
  ...
  const key = secretKey || apiKey || "";
  if ((0, import_shared.isProductionFromApiKey)(key) && !clientUat && !cookieToken) {
...
function assertSignInUrlExists(signInUrl, key) {
  if (!signInUrl && (0, import_shared.isDevelopmentFromApiKey)(key)) {
...
async function authenticateRequest(options) {
  ...
  options = {
    ...
    frontendApi: (0, import_shared.parsePublishableKey)(options.publishableKey)?.frontendApi || options.frontendApi,
...
function redirect({ redirectAdapter, signUpUrl, signInUrl, frontendApi, publishableKey }) {
  if (!frontendApi) {
    frontendApi = (0, import_shared.parsePublishableKey)(publishableKey)?.frontendApi;
  } else {
    (0, import_shared.deprecated)("frontendApi", "Use `publishableKey` instead.");
  }
...
function Clerk(options) {
  ...
  (0, import_shared21.deprecatedObjectProperty)(
    clerkInstance,
    "__unstable_options",
