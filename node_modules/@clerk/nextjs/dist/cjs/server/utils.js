...
const handleMultiDomainAndProxy = (req, opts) => {
  ...
  const relativeOrAbsoluteProxyUrl = (0, import_shared.handleValueOrFn)(opts?.proxyUrl, requestURL, import_clerkClient.PROXY_URL);
  let proxyUrl;
  if (!!relativeOrAbsoluteProxyUrl && !(0, import_shared.isHttpOrHttps)(relativeOrAbsoluteProxyUrl)) {
    proxyUrl = new URL(relativeOrAbsoluteProxyUrl, requestURL).toString();
  } else {
    proxyUrl = relativeOrAbsoluteProxyUrl;
  }
  const isSatellite = (0, import_shared.handleValueOrFn)(opts.isSatellite, new URL(req.url), import_clerkClient.IS_SATELLITE);
  const domain = (0, import_shared.handleValueOrFn)(opts.domain, new URL(req.url), import_clerkClient.DOMAIN);
  const signInUrl = opts?.signInUrl || import_clerkClient.SIGN_IN_URL;
  if (isSatellite && !proxyUrl && !domain) {
    throw new Error(import_errors.missingDomainAndProxy);
  }
  if (isSatellite && !(0, import_shared.isHttpOrHttps)(signInUrl) && isDevelopmentFromApiKey(import_clerkClient.SECRET_KEY || import_clerkClient.API_KEY)) {
